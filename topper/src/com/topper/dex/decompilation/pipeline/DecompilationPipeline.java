...
public final class DecompilationPipeline {
	...
	/**
	 * Sweeper that extracts instruction sequences from a given byte array.
	 */
	private Sweeper sweeper;

	/**
	 * Analyser responsible for perform static analysis. This involves CFG and DFG
	 * extraction.
	 */
	private StaticAnalyser staticAnalyser;

	/**
	 * Analyser for semantical tasks. E.g. if a gadget is contradictory to the task,
	 * or behaves like a NOP, it can be filtered.
	 */
	private SemanticAnalyser semanticAnalyser;

	/**
	 * Initializes this pipeline with default stages. Currently the default stages
	 * are: - <code>BackwardLinearSweeper</code> -
	 * <code>DefaultStaticAnalyser</code> - <code>DefaultSemanticAnalysr</code>
	 * 
	 * The default values can be overwritten by calling their respective setters.
	 * 
	 ...
	 * 
	 * There are currently three different stages in this pipeline: 1. Sweeping:
	 * Detect valid sequences of instructions. 2. Static Analysis: For each
	 * instruction sequence extract the CFG and DFG and construct a gadget. 3.
	 * Semantic Analysis: For each gadget add further information by conducting e.g.
	 * a symbolic analysis. This stage may filter gadgets e.g. in case a gadget is
	 * contradictory.
	 * 
	 * Currently, it is possible to skip a stage by supplying -
	 * <code>EmptySweeper</code> to <code>setSweeper</code> -
	 * <code>EmptyCFGAnalyser</code> and <code>EmptyDFGAnalyser</code> to an
	 * instance of <code>StaticAnalyser</code> and then providing that instance to
	 * <code>setStaticAnalyser</code> - <code>EmptySemanticAnalyser</code> to
	 * <code>setSemanticAnalyser</code> If e.g. <code>EmptySweeper</code> is used,
	 * then the sweeper will not find any instruction sequences and thus all
	 ...
	 * 
	 * Finally, using <code>EmptySemanticAnalyser</code> filters all gadgets, thus
	 * resulting in this method returning an empty list.
	 * 
	 * @param bytes  Byte array, from which to extract gadgets.
	 * @param offset Offset relative to the beginning of <code>bytes</code>. It
	 *               represents the starting point of decompilation.
	 * @return List of gadgets extracted from <code>bytes</code> by applying above
	 *         mentioned stages.
	...
	public final ImmutableList<@NonNull Gadget> decompile(final byte @NonNull [] bytes, final int offset)
		...
		final ImmutableList<@NonNull ImmutableList<@NonNull DecompiledInstruction>> instructionSequences = this.sweeper
				.sweep(bytes, offset);
		...
		Gadget gadget;
		for (@NonNull
		final Gadget candidate : gadgets) {
